...
static uint8_t pinRx = 2;		// Le numéro de la broche IO utilisée pour le module récepteur
...
// La somme des valeurs des samples. si inférieur é 2 "1" samples dans le cycle de PLL
...
volatile unsigned long last_check = 0;
...
ydlelib::ydlelib(int rx, int tx, int button)
{
	m_bLnkSignalReceived = false;
	m_initializedState = false;
...
void ydlelib::init_timer(){
	Timer1.initialize(128); // set a timer of length 128 microseconds
...
uint8_t ydlelib::crc8(const uint8_t* buf, uint8_t length) {
...
unsigned char ydlelib::computeCrc(Frame_t* frame){
	...
	if(wait_ack == 1){
		if(retry <= 3){
			...
			if(curt - last_check >= YDLE_ACK_TIMEOUT){
				last_check = curt;
				g_sendFrameBuffer.crc = 0;
				--g_sendFrameBuffer.taille;
...
void ydlelib::handleReceivedFrame(Frame_t *frame)
{
	int litype;
	long livalue;
	...
	if(this->extractData(frame, 0,litype, livalue)==1)
	{
		...
		Serial.print("Duree de traitement ");
...
void ydlelib::send(Frame_t *frame)
{
	int i = 0,j = 0;
	...
	Timer1.stop();
 	...
 	for (i=0; i<16; i++)
 	{
 		...
 		delayMicroseconds(YDLE_TPER);     // un bit à l'état haut
 		digitalWrite(pinTx, LOW);
 		delayMicroseconds(YDLE_TPER);     // un bit à l'état bas
	}
...
bool ydlelib::checkSignal(Frame_t *frame)
{
...
void ydlelib::dataToFrame(Frame_t *frame, unsigned long destination, unsigned long sender, unsigned long type)
{
	frame->sender = sender;
...
void ydlelib::dataToFrame(Frame_t *frame, unsigned long type)
{
...
bool ydlelib::initialized()
{
	return m_initializedState;
}

int ydlelib::isSignal()
{
	return rx_active;
}
...
*/
// ----------------------------------------------------------------------------
void ydlelib::addCmd(Frame_t *frame, int type, int data)
{
...
*/
// ----------------------------------------------------------------------------
int ydlelib::extractData(Frame_t *frame, int index, int &itype, long &ivalue)
{
	uint8_t* ptr;
...
*/
// ----------------------------------------------------------------------------
void ydlelib::addData(Frame_t * frame, int type, long data)
{
...
// Affiche les logs sur la console série
void ydlelib::log(String msg,int i)
{
#if not defined( __AVR_ATtiny85__ ) or defined(_YDLE_DEBUG)
	Serial.print(msg);
	Serial.println(i);
...
*/
// ----------------------------------------------------------------------------
void ydlelib::printFrame(Frame_t *trame)
{
#if not defined( __AVR_ATtiny85__ ) or defined (_YDLE_DEBUG)
	// if debug
		char sztmp[255];
